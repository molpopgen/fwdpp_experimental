...
{
    double pos, s, h;
    unsigned gen, references;
    bool neutral, fixed;
    mutation(const double &__p, const double &__s, const double &__h,
             const unsigned &__g)
        : pos(__p), s(__s), h(__h), gen(__g), references{}, neutral(s == 0.),
          fixed(false)
    {
    }
};
...
struct gamete
{
    ...
    unsigned references;
    gamete() : mutations(mvector_t()), smutations(mvector_t()), references{}
    {
        mutations.reserve(1000);
        smutations.reserve(1000);
    }
    gamete(const mvector_t &n, const mvector_t &s)
        : mutations(n), smutations(s), references{}
    {
        mutations.reserve(1000);
        smutations.reserve(1000);
    }
...
using gptr_t = boost::intrusive_ptr<gamete>;
...
template <typename iterator_type, typename vector_type_allocator,
    ...
    gvector_t *gametes, iterator_type &g1, iterator_type &g2)
{
    ...
    // ALLOCATE
    iterator_type new_gamete1 = gptr_t(new gamete()),
                  new_gamete2 = gptr_t(new gamete());
...
template <typename iterator_type, typename recombination_map,
          typename gvector_t>
unsigned
recombine_gametes(gsl_rng *r, const double &littler, gvector_t *gametes,
                  iterator_type &g1, iterator_type &g2,
...
template <typename diploid_fitness_function, typename mutation_removal_policy,
...
double
sample_diploid(gsl_rng *r, gvector_t *gametes, dipvector_t *diploids,
               mvector_t *mutations, const unsigned &N_curr,
               ...
               const mutation_removal_policy &mp, const double &f)
{
    ...
    gametes->erase(std::remove_if(gametes->begin(), gametes->end(),
                                  [](typename gvector_t::value_type &__g) {
                                      return __g->references == 1;
                                  }),
                   gametes->end());
    // uh-oh
    std::for_each(
        mutations->begin(), mutations->end(), [&diploids](mptr_t &__m) {
            if (unsigned(__m->references) == 2u * diploids->size() + 1u)
                __m->fixed = true;
        });
...
struct mpolicy
{
    ...
    operator()(gsl_rng *r, lookup_t *lookup, const unsigned &generation) const
    {
        ...
        return result_type(new mutation(pos, 0., 0., generation));
    }
};
...
main(int argc, char **argv)
{
    ...
    gvector gametes(1, gptr_t(new gamete())); // monomorphic pop
