...
struct mutation
{
    ...
    unsigned gen;
    bool neutral, fixed;
    mutation(const double &__p, const double &__s, const double &__h,
             const unsigned &__g)
        : pos(__p), s(__s), h(__h), gen(__g), neutral(s == 0.), fixed(false)
    {
    }
...
struct gamete
{
    ...
    gamete(const mvector_t &n, const mvector_t &s)
        : mutations(n), smutations(s)
    {
        mutations.reserve(1000);
        smutations.reserve(1000);
    }
    ...
    gamete(gamete *g)
        : mutations(std::move(g->smutations)),
...
// Below, stuff is basically copied right from fwdpp, but is hacked to avoid
// tag/dispatch stuff
...
struct mutation_remover
{
    ...
    inline result_type
    operator()(const iterator_type &i, const unsigned &x1) const
    {
    ...
    template <typename iterator_type>
    inline result_type
    operator()(const iterator_type &i, const unsigned &x1,
...
template <typename gamete_type, typename mutation_iterator>
void
add_new_mutation(mutation_iterator mitr, gamete_type &new_gamete)
{
    if (mitr->neutral)
        {
            new_gamete->mutations.insert(
                std::lower_bound(
                    new_gamete->mutations.begin(), new_gamete->mutations.end(),
                    mitr->pos,
                    [](const mutation_iterator &__mut, const double &__value) {
        ...
        {
            new_gamete->smutations.insert(
                std::lower_bound(
                    new_gamete->smutations.begin(),
...
template <typename mutation_model, typename mutation_insertion_policy,
          typename mlist_type, typename gamete_type>
void
add_N_mutations(const mutation_model &mmodel,
                ...
                mlist_type *mutations, gamete_type &g)
{
    for (unsigned i = 0; i < n; ++i)
        {
            // ALLOCATE
            // auto nmut_p = typename mlist_type::value_type(new
            // mutation(mmodel()));
            auto nmut_p = std::allocate_shared<decltype(mmodel())>(
                std::allocator<decltype(mmodel())>(), mmodel());
...
template <typename gamete_t, typename mutation_model, typename mvector_t,
          typename gvector_t, typename mutation_insertion_policy,
          typename gamete_insertion_policy>
gamete_t
mutate_gamete(gsl_rng *r, const double &mu, gvector_t *gametes,
              mvector_t *mutations, gamete_t &g, const mutation_model &mmodel,
    ...
    if (nm)
        {
            ...
            gamete_t ng = std::allocate_shared<gamete>(
                std::allocator<gamete>(), gamete(*g));
...
namespace fwdpp_internal
{
    ...
    template <typename itr_type, typename cont_type>
    itr_type
    rec_gam_updater(itr_type &__first, itr_type &__last, cont_type &m1,
                    cont_type &m2, const short &SWITCH, const double &val)
    {
        // O(log_2) comparisons of double plus at most __last - __first copies
        itr_type __ub = std::lower_bound(
            __first, __last, std::cref(val),
            [](const typename itr_type::value_type &__mut,
               const double &__val) { return __mut->pos < __val; });
        if (SWITCH)
    ...
    template <typename double_vec_type, typename gamete_type,
              typename gamete_cont_iterator>
    void
    recombine_gametes(const double_vec_type &pos, gamete_cont_iterator &ibeg,
                      gamete_cont_iterator &jbeg, gamete_type &new_gamete1,
                      gamete_type &new_gamete2)
    {
        assert(std::is_sorted(pos.cbegin(), pos.cend()));
        short SWITCH = 0;

        auto itr = ibeg->mutations.cbegin(), jtr = jbeg->mutations.cbegin(),
             itr_s = ibeg->smutations.cbegin(),
             jtr_s = jbeg->smutations.cbegin(), itr_e = ibeg->mutations.cend(),
             itr_s_e = ibeg->smutations.cend(), jtr_e = jbeg->mutations.cend(),
             jtr_s_e = jbeg->smutations.cend();
#ifndef NDEBUG
        decltype(ibeg->mutations.size()) nm1
            = ibeg->mutations.size() + ibeg->smutations.size(),
        ...
        for (const auto &dummy : pos)
            {
                ...
                itr = fwdpp_internal::rec_gam_updater(
                    itr, itr_e, new_gamete2->mutations, new_gamete1->mutations,
                    SWITCH, dummy);
                itr_s = fwdpp_internal::rec_gam_updater(
                    itr_s, itr_s_e, new_gamete2->smutations,
                    new_gamete1->smutations, SWITCH, dummy);
                jtr = fwdpp_internal::rec_gam_updater(
                    jtr, jtr_e, new_gamete1->mutations, new_gamete2->mutations,
                    SWITCH, dummy);
                jtr_s = fwdpp_internal::rec_gam_updater(
                    jtr_s, jtr_s_e, new_gamete1->smutations,
                    new_gamete2->smutations, SWITCH, dummy);
                SWITCH = !SWITCH;
            }
#ifndef NDEBUG
        decltype(new_gamete1->mutations.size()) __nm1
...
/*
  ...
  typename gamete_type::mutation_list_type_iterator rhs) { return lhs->pos <
  rhs->pos; });
  std::sort(new_gamete1.smutations.begin(),new_gamete1.smutations.end(),
  [](typename gamete_type::mutation_list_type_iterator lhs,
  typename gamete_type::mutation_list_type_iterator rhs) { return lhs->pos <
...
*/
        ...
        // PROBLEM!
        auto am_I_sorted
            = [](mptr_t &lhs, mptr_t &rhs) { return lhs->pos < rhs->pos; };
        assert(std::is_sorted(new_gamete1->mutations.begin(),
                              new_gamete1->mutations.end(),
                              std::cref(am_I_sorted)));
        assert(std::is_sorted(new_gamete1->smutations.begin(),
                              new_gamete1->smutations.end(),
                              std::cref(am_I_sorted)));
        assert(std::is_sorted(new_gamete2->mutations.begin(),
                              new_gamete2->mutations.end(),
                              std::cref(am_I_sorted)));
        assert(std::is_sorted(new_gamete2->smutations.begin(),
                              new_gamete2->smutations.end(),
                              std::cref(am_I_sorted)));
#endif
    }
}
...
template <typename iterator_type,
          ...
          template <typename, typename> class vector_type, typename gvector_t>
// template<typename,typename> class list_type>
unsigned
recombine_gametes_details(
    const vector_type<double, vector_type_allocator> &pos,
    // list_type< typename iterator_type::value_type,list_type_allocator > *
    // gametes,
    gvector_t *gametes, iterator_type &g1, iterator_type &g2)
{
    ...
    iterator_type new_gamete1
        = std::allocate_shared<gamete>(std::allocator<gamete>(), gamete()),
        new_gamete2
        = std::allocate_shared<gamete>(std::allocator<gamete>(), gamete());
    // gtype new_gamete1(0u,gtype_mcont(),gtype_mcont()),
    //  new_gamete2(new_gamete1);

    new_gamete1->mutations.reserve(g1->mutations.size()
                                   + g2->mutations.size());
    new_gamete1->smutations.reserve(g1->smutations.size()
                                    + g2->smutations.size());
    new_gamete2->mutations.reserve(g1->mutations.size()
                                   + g2->mutations.size());
    new_gamete2->smutations.reserve(g1->smutations.size()
                                    + g2->smutations.size());
    ...
    for (auto itr = gametes->begin(); (!f1 || !f2) && itr != current_end;
         ++itr)
        {
...
template <typename iterator_type, typename recombination_map,
          // typename list_type_allocator,
          typename gvector_t>
// template<typename,typename> class list_type>
unsigned
recombine_gametes(gsl_rng *r, const double &littler,
                  // list_type< typename
                  // iterator_type::value_type,list_type_allocator > * gametes,
                  gvector_t *gametes, iterator_type &g1, iterator_type &g2,
                  const recombination_map &mf)
{
    ...
    // Identify cases where recombination cannot result in changed gametes, and
    // get out quick
    if (g1 == g2)
        return 0;
...
struct genetics101
/*! Genetics 101: simple model of recombination.  r is the probability that the
 * two gametes recombine
 */
{
    using result_type = unsigned;
    template <typename gamete_iterator_type, typename gvector_t,
              // typename gamete_list_type_allocator,
              // template<typename,typename> class gamete_list_type,
              typename rec_pos_generator>
    unsigned
    operator()(gamete_iterator_type &g1, gamete_iterator_type &g2,
               // gamete_list_type< typename gamete_iterator_type::value_type,
               // gamete_list_type_allocator > * gametes,
               gvector_t *gametes, const double &littler, gsl_rng *r,
               const rec_pos_generator &rp) const
    {
        unsigned NREC = 0;
        if (g1 != g2)
            // then a non-parental type is inherited from p1 and p1 has two
            // different gametes
            {
...
struct no_selection
{
    ...
    */
    ...
    template <typename iterator_type>
    inline result_type
    operator()(const iterator_type &, const iterator_type &) const
    {
        /*
          static_assert( std::is_base_of<mutation_base,
          typename iterator_type::value_type::mutation_type>::value,
          "iterator_type::value_type::mutation_type must be derived from
          KTfwd::mutation_base" );
        */
        return 1.;
    }
...
struct site_dependent_fitness
{
    ...
    template <typename iterator_type, typename fitness_updating_policy_hom,
              typename fitness_updating_policy_het>
    inline result_type
    operator()(const iterator_type &g1, const iterator_type &g2,
               ...
               const double &starting_fitness = 1.) const
    {
        ...
        */
        ...
        if (!g1->smutations.empty() && g2->smutations.empty())
            {
                std::for_each(g1->smutations.begin(), g1->smutations.end(),
                              std::bind(fpol_het, std::ref(fitness),
                                        std::placeholders::_1));
                return fitness;
            }
        if (g1->smutations.empty() && !g2->smutations.empty())
            {
                std::for_each(g2->smutations.begin(), g2->smutations.end(),
                              std::bind(fpol_het, std::ref(fitness),
                                        std::placeholders::_1));
                return fitness;
            }
        typename iterator_type::value_type::mutation_list_type_iterator ib1,
            ib2;
        typename iterator_type::value_type::mutation_container::const_iterator
            b1
            = g1->smutations.cbegin(),
            e1 = g1->smutations.cend(), b2 = g2->smutations.cbegin(),
                ...
                for (; !found && b2 < e2 && !((*b2)->pos > (ib1)->pos); ++b2)
                    {
                        ...
                        if (ib2 == ib1) // homozygote
                            ...
                            // b2 points to a unique mutation that comes before
                            // b1
                            {
        ...
        std::for_each(b2, e2, std::bind(fpol_het, std::ref(fitness),
                                        std::placeholders::_1));
        return fitness;
    }
};
...
template < // typename gamete_type,
    ...
    typename diploid_fitness_function, typename mutation_removal_policy,
    typename mutation_model, typename recombination_policy,
    typename mutation_insertion_policy, typename gamete_insertion_policy,
    typename gvector_t, typename mvector_t, typename dipvector_t>
// template<typename,typename> class gamete_list_type,
// template<typename,typename> class mutation_list_type,
// template<typename,typename> class diploid_vector_type>
double
sample_diploid(
    gsl_rng *r, gvector_t *gametes, dipvector_t *diploids,
    mvector_t *mutations,
    ...
    // diploids,
    // mutation_list_type<typename
    // gamete_type::mutation_type,mutation_list_type_allocator > * mutations,
    const unsigned &N_curr, const unsigned &N_next, const double &mu,
    const mutation_model &mmodel, const recombination_policy &rec_pol,
    const mutation_insertion_policy &mpolicy,
    const gamete_insertion_policy &gpolicy_mut,
    const diploid_fitness_function &ff, const mutation_removal_policy &mp,
    const double &f)
{
    assert(N_curr == diploids->size());

    // std::for_each( mutations->begin(),mutations->end(),[](typename
    // gamete_type::mutation_type & __m){__m.n=0;});
    ...
    for (unsigned i = 0; i < N_curr; ++i)
        {
            ...
            //(dptr+i)->second->n = 0;
            // fitnesses[i] =
            // fwdpp_internal::diploid_fitness_dispatch(ff,(dptr+i),
            //								typename
            //traits::is_custom_diploid_t<diploid_geno_t>::type());
    ...
    /*
  #ifndef NDEBUG
    std::for_each(gametes->cbegin(),gametes->cend(),[](decltype((*gametes->cbegin()))
  __g) {
        assert( !__g.n ); } );
  #endif
  */
    ...
    for (unsigned i = 0; i < N_next; ++i)
        {
            ...
            size_t p2 = (gsl_rng_uniform(r) <= f)
                            ? p1
                            : gsl_ran_discrete(r, lookup.get());
#else
            size_t p2 = (f == 1. || (f > 0. && gsl_rng_uniform(r) <= f))
                            ? p1
                            : gsl_ran_discrete(r, lookup.get());
#endif
            ...
            p1g1
                = (pptr + typename decltype(pptr)::difference_type(p1))->first;
            p1g2 = (pptr + typename decltype(pptr)::difference_type(p1))
                       ->second;
            p2g1
                = (pptr + typename decltype(pptr)::difference_type(p2))->first;
            p2g2 = (pptr + typename decltype(pptr)::difference_type(p2))
                       ->second;
            ...
            (dptr + i)->first
                = mutate_gamete(r, mu, gametes, mutations, (dptr + i)->first,
                                mmodel, mpolicy, gpolicy_mut);
            (dptr + i)->second
                = mutate_gamete(r, mu, gametes, mutations, (dptr + i)->second,
                                mmodel, mpolicy, gpolicy_mut);
        }
    // #ifndef NDEBUG
    ...
    gametes->erase(std::remove_if(gametes->begin(), gametes->end(),
                                  [](typename gvector_t::value_type &__g) {
                                      return __g.use_count() == 1;
                                  }),
                   gametes->end());
    // uh-oh
    std::for_each(
        mutations->begin(), mutations->end(), [&diploids](mptr_t &__m) {
            if (unsigned(__m.use_count()) == 2u * diploids->size() + 1u)
                __m->fixed = true;
        });
    std::for_each(gametes->begin(), gametes->end(),
                  [&mp](decltype(*(gametes->begin())) &__g) {
                      __g->mutations.erase(
                          std::remove_if(__g->mutations.begin(),
                                         __g->mutations.end(), std::cref(mp)),
                          __g->mutations.end());
                      __g->smutations.erase(
                          std::remove_if(__g->smutations.begin(),
                                         __g->smutations.end(), std::cref(mp)),
                          __g->smutations.end());
                  });
    // decltype(gametes->begin()) temp;
    // for( auto itr = gametes->begin() ; itr != gametes->end() ;  )
    //   {
    //     if( itr->n == 0 ) //this gamete is extinct and need erasing from the
    //     list
    // 	{
    // 	  temp = itr;
    // 	  ++itr;
    ...
    // 		 [&mp](decltype( *(gametes->begin()) ) & __g ) {
    // 		   __g.mutations.erase(
    // std::remove_if(__g.mutations.begin(),__g.mutations.end(),std::cref(mp)),__g.mutations.end()
    // );
    // 		   __g.smutations.erase(
    // std::remove_if(__g.smutations.begin(),__g.smutations.end(),std::cref(mp)),__g.smutations.end()
    // );
    // 		 });
...
struct mpolicy
{
    ...
    inline result_type
    operator()(gsl_rng *r, lookup_t *lookup, const unsigned &generation) const
    {
...
main(int argc, char **argv)
{
    ...
    for_each(diploids.begin(),diploids.end(),
             [](const diploid_t & __d) {
               cerr << __d.first->smutations.size() << ' ' <<
    __d.first->smutations.size() << ' '
                    << __d.second->smutations.size() << ' ' <<
    __d.second->smutations.size() << ' '
                    << __d.first.use_count() << ' ' << __d.second.use_count()
    << '\n';
             });
    */
    // Here is the minor down-side: the shared_ptr is 2x the size of an
    // interator, presumable b/c it stores other info.
    // cerr << sizeof(mptr_t) << ' ' << sizeof(gptr_t) << ' ' << sizeof(
    // vector<mutation>::iterator ) << ' ' << sizeof(list<mutation>::iterator)
    // << '\n';
    ...
    //   const double & mu, list_type< typename
    //   iterator_type::value_type,list_type_allocator > * gametes,
    //   list_type2<typename
    //   iterator_type::value_type::mutation_type,list_type_allocator2 > *
    //   mutations,
    //   iterator_type & g,
    ...
    // 				    []( mptr_t && m, mvector * mv) { return
    // mv->insert(mv->end(),forward<mptr_t>(m)); },
    // 				    []( gptr_t && g, gvector * gv) { return
    // *(gv->insert(gv->end(),forward<gptr_t>(g))); }
    // 				    );
    ...
    //   {
    //     cout << g->use_count() << ' ' << (*g)->mutations.size() << ' ' <<
    //     (*g)->smutations.size() << '\n';
    ...
    // unsigned NREC =
    // genetics101()(diploids[0].first,diploids[0].second,&gametes,littler,r,recmap);
    ...
    //[]( mutation && __m, mvector * mv ) { mv->emplace_back(mptr_t(new
    //mutation(std::forward<mutation>(__m)))); return mv->end()-1; },
    //[]( gamete && __g, gvector * gv ) { gv->emplace_back(mptr_t(new
    //gamete(std::forward<gamete>(__g)))); return gv->end()-1; } );

    for (unsigned gen = 0; gen < simlen; ++gen)
        {
            // if(gen % 100 == 0.) cout << gen << endl;
            double wbar = sample_diploid(
                r, &gametes, &diploids, &mutations,
                // constant size...
                N, N,
                // Mutation
                mu, std::bind(mpolicy(), r, &lookup, gen),
                // crossing-over
                ...
                [](gptr_t &&g, gvector *gv) {
                    return *(gv->insert(gv->end(), forward<gptr_t>(g)));
                },
                // Fitness
                std::bind(no_selection(), std::placeholders::_1,
                          std::placeholders::_2),
                // mut removal policy
